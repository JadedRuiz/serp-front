...
export class CatalogoComponent implements OnInit {
  ...
  siguienteSlide() {
    this.inicoSlide =
      (this.inicoSlide + 1) % this.selectedCard?.imagenes.length;
  }

  anteriorSlide() {
    this.inicoSlide =
      (this.inicoSlide - 1 + this.selectedCard?.imagenes.length) %
      this.selectedCard?.imagenes.length;
  }
  ...
  itemsFiltrados() {
    this.filteredItems = this.articulos.filter((articulo) => {
      return this.familiasActivas.some(
        (familia) => familia.id_familia === articulo.id_familia
      );
    });
    ...
    this.catalogo.obtenerArticulos().subscribe((resp) => {
      if (resp.ok) {
        ...
        this.familias.obtenerFamilias(json).subscribe((resp) => {
          ...
          this.familiasActivas = familias.filter(
            (familia: Familia) => familia.activo == 1
          );
          this.itemsFiltrados();
        });
      } else {
  ...
  agregarProductoCarrito(item: Articulo) {
    const productoExistente = this.pedido.find(
      (product) => product.id_existencia === item.id_existencia
    );
  ...
  buscar() {
    if (this.searchTitle === '' && this.searchFam === '') {
      ...
      this.filteredItems = this.filteredItems2.filter(
        (articulo) =>
          articulo.articulo
            .toLowerCase()
