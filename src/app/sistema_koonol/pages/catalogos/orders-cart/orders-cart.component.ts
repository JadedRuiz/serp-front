...
export class OrdersCartComponent {
    ...
    ngOnInit(): void {
        ...
        this.catalogo.pedido$.subscribe(pedido => {
            ...
            this.pedido.forEach(articulo => {
                this.pedidoFinal.articulos.push(new ArticuloPedido(0, articulo.id_existencia, articulo.precio_venta, articulo.costo_promedio, articulo.quantity, articulo.tasa_iva, articulo.precio_total));
            })
            this.pedidos.updatePedidoFinal(this.pedidoFinal)
            console.log(this.pedidoFinal.articulos);
        })
        this.pedidos.pedidoFinal$.subscribe(
            pedidoFinal => {
    ...
    getTotalPerItem() {
        this.pedido.forEach(articulo => {
            ...
            articulo.precio_total = (Number(articulo.precio_venta) + articuloIVA) * articulo.quantity
        })
    }
